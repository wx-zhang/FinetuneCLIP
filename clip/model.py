...
class ModifiedResNet(nn.Module):
    ...
    def forward(self, x, prompts=None, deep_prompts=None):
        def stem(x):
...
class Transformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, deep_prompt=None):
        if deep_prompt is not None:
            hidden_states = None
            B = x.shape[1]
            num_layers = len(self.resblocks)
            num_tokens = 5
            for layer_num, layer in enumerate(self.resblocks):
                if layer_num == 0:
                    hidden_states = layer(x)
                else:
                    deep_prompt_emb = deep_prompt[layer_num -
                                                  1].expand(B, -1, -1).permute(1, 0, 2)
                    hidden_states = torch.cat((
                        hidden_states[:1, :, :],
                        deep_prompt_emb,
                        hidden_states[(1+num_tokens):, :, :]
                    ), dim=0)
                    hidden_states = layer(x)
...
class VisionTransformer(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int, output_dim: int):
        ...
        # ! This has to be changed to prompted vision transformer based on arguments
        self.transformer = Transformer(width, layers, heads)

        self.ln_post = LayerNorm(width)
        # ! We might wanna learn this too
        self.proj = nn.Parameter(scale * torch.randn(width, output_dim))

    def forward(self, x: torch.Tensor, prompts=None, deep_prompts=None):
        ...
        x = x + self.positional_embedding.to(x.dtype)  # here

        # ! skeptical about this pre-norm and post norm - change if not works
        x = self.ln_pre(x)

        x = x.permute(1, 0, 2)  # NLD -> LND
        x = self.transformer(x)  # ! This is the encoder in there
...
class CLIP(nn.Module):
    ...
    def encode_image(self, image, prompts=None, deep_prompts=None):
        return self.visual(image.type(self.dtype), prompts=prompts, deep_prompts=deep_prompts)

    def encode_image_normalize(self, image):
...
def build_model(state_dict: dict):
    ...
    wsg = model.load_state_dict(state_dict, strict=False)
    print(wsg)
    return model.eval()
